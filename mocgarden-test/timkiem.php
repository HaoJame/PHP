				...
				<div class="box-tk-gh">
					...
					<div class="container-cart">
						...
						<li class="dropdown">
								...
								<span class="inf"><font size="3">Tổng:
									<?php 
								 ...
								 <ul class="dropdown-menu cart-list"> 
									 <?php 
											...
											if(isset($_SESSION['giohang'])){
												...
												{
													/*$result = chayTruyVanTraVeDL($link, "select * from tbl_hinhanhsanpham where id_sanpham=".$v['id']);
													while($rows=mysqli_fetch_assoc($result))
													{
														$hinhanh = $rows['hinhanh_sanpham'];
														break;	
													}*/
													
													<li style='margin-left: auto;margin-right: auto; width: 95%'> 
													  <a href='#' class='photo' style='width: 25%; float:left'>
														  <img src='pic/sanpham/luoi-ho-1.jpg' class='cart-thumb' style='width: 100%'>
													  </a> 
													  <span class='pro-info' style='width: 50%; margin-left: 15px; display: inline-block; margin-bottom: 10px'>
														  <a href='#' class='pro-name'> <?php echo $v['ten'];?></a> <br>
														  <span class='price'> <?php echo $v['gia'];?></span>
														  <br>
														 
														  <div class='btn_tanggiam'>
															  <form method='post' action='xulygiohang.php'>
															  <input class='minus is-form' type='submit' value='-' name='action'>
															  <input aria-label='quantity' class='input-qty' max='1000' min='1' type='number' value='<?php echo $v['soluong'];?>' name='soluong'>
															  <input class='plus is-form' type='submit' value='+' name='action'>
															  <input type='hidden' value='".$v['id']."' name='id_sanpham'>
															</form>
															</div>
					...
					<!--Hộp tìm kiếm-->
					<form class="container-tk" method="get" action="timkiem.php">
						<input type="search" id="search" placeholder="Tìm kiếm sản phẩm..." name="keyword" />
						<a href="timkiem.html"><button type="submit" class="search" value="Tìm kiếm"><i class="fa fa-search"></i></button></a>
  					</form>
		...
		<div class="row">
			...
			<div class="col-12 col-md-6 bg-dark">
				...
				<div class="dk-email">
					...
					<form method="get" action="xulydkmail.php">
						<input type='email' placeholder='  Nhập email của bạn...' name='email' id='mail' aria-label='general-newsletter_form-newsletter_email'> 
						<button class="btn_dki" type="submit" name="subscribe" id="subscribe" data-toggle="modal" data-target="#myModal"><b>Đăng ký</b></button>
					 </form>
					...
					<?php 
						...
						{
							if ($_GET['msg']=="done")
							{ 	echo "<h7><font color='yellow'><b><i>Cảm ơn bạn đã quan tâm Mộc Garden!</i></b></font></h7>";
						?>
							...
							<div class='modal-dialog'>
								<div class='modal-content'>
									...
									<div class='modal-body'>
										Cảm ơn bạn đã quan tâm đến Mộc Garden.
									</div>
